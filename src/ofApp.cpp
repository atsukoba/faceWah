...
glm::vec2 ofApp::getFacePoint(int i) {
    ...
    // (numbering https://ibug.doc.ic.ac.uk/resources/frog-facial-tracking-component-code/)
...
void ofApp::draw(){
    ...
    if (isMirror) {
        tracker.drawDebug(grabber.getWidth(), 0, -grabber.getWidth(), grabber.getHeight());
    } else {
        tracker.drawDebug();
    }
    ...
    ofDrawLine(getFacePoint(57).x,
               getFacePoint(57).y,
               getFacePoint(51).x,
               getFacePoint(51).y);
    ofSetColor(ofColor(0, 0, 255));
    ofDrawLine(getFacePoint(48).x,
               getFacePoint(48).y,
               getFacePoint(54).x,
               getFacePoint(54).y);
